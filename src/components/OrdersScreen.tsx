...
function OrderCard({order,onEdit,onDuplicate,onStatus}:{order:Order;onEdit:()=>void;onDuplicate:()=>void;onStatus:(s:OrderStatus)=>void}) {
  ...
  return <View style={s.card}>
    ...
    <View style={s.facts}><View><Text style={s.factLabel}>{"DUE DATE"}</Text><Text style={[s.factValue,due&&{color:due.color}]}>{displayDate(order.target_date)||"Not set"}</Text></View><View><Text style={s.factLabel}>PRICE</Text><Text style={s.factValue}>{peso(Number(order.total_price))}</Text></View><View><Text style={s.factLabel}>BALANCE</Text><Text style={s.factValue}>{peso(Math.max(0,balance))}</Text></View></View>
    {due && due.color!==C.green?<Text style={{marginTop:9,color:due.color,fontSize:13}}>{due.text}</Text>:null}{stage!=="full"?<Text style={[s.depositLine,{color:stageColor}]}>{paymentMessage}</Text>:null}
    {showDetails && order.amount_paid>0?<Text style={s.paymentDetail}>{peso(Number(order.amount_paid))} paid via {order.payment_channel}{balance>0?` · ${peso(balance)} remaining`:""}</Text>:null}
...
const s=StyleSheet.create({
  ...
  hero:{marginTop:18,padding:16,borderWidth:1,borderColor:"#DCE5EB",borderRadius:14,backgroundColor:"#EEF3F7"},heroKicker:{color:C.navy,fontSize:11,fontWeight:"700",letterSpacing:1.6},heroValue:{marginTop:6,color:C.ink,fontSize:34,fontWeight:"700"},heroHelp:{marginTop:5,color:C.muted,fontSize:14,fontWeight:"600"},tabs:{gap:8,paddingVertical:14},tab:{minHeight:44,paddingHorizontal:15,alignItems:"center",justifyContent:"center",borderWidth:1,borderColor:C.border,borderRadius:10,backgroundColor:C.white},tabOn:{backgroundColor:C.ink,borderColor:C.ink},tabText:{color:C.ink,fontSize:13,fontWeight:"700"},tabTextOn:{color:C.white},
  ...
  card:{flex:1,marginTop:0,padding:14,borderWidth:1,borderColor:C.border,borderRadius:14,backgroundColor:C.white},cardTop:{flexDirection:"row",gap:12},photo:{width:52,height:52,borderRadius:11,resizeMode:"contain",backgroundColor:C.pale},noPhoto:{width:52,height:52,alignItems:"center",justifyContent:"center",borderRadius:11,backgroundColor:C.pale},cardMain:{flex:1,minWidth:0},numberRow:{flexWrap:"wrap",flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:6},number:{color:C.muted,fontSize:12,fontWeight:"700",letterSpacing:.5},status:{paddingHorizontal:9,paddingVertical:6,flexDirection:"row",alignItems:"center",gap:4,borderRadius:12,backgroundColor:C.pale},statusHold:{backgroundColor:"#FBEFF1"},statusText:{color:C.navy,fontSize:12,fontWeight:"700"},cardTitle:{marginTop:7,color:C.ink,fontSize:16,lineHeight:21,fontWeight:"700"},meta:{marginTop:4,color:C.muted,fontSize:14,lineHeight:20,fontWeight:"500"},pastOrderTag:{alignSelf:"flex-start",marginTop:7,paddingHorizontal:8,paddingVertical:4,flexDirection:"row",alignItems:"center",gap:4,borderRadius:10,backgroundColor:"#F8F2E8"},pastOrderTagText:{color:C.amber,fontSize:10,fontWeight:"800",letterSpacing:.6},paymentTag:{alignSelf:"flex-start",minHeight:32,marginTop:10,paddingHorizontal:11,flexDirection:"row",alignItems:"center",gap:6,borderRadius:15},paymentTagText:{fontSize:13,fontWeight:"700"},priceNeededTag:{alignSelf:"stretch",minHeight:42,marginTop:10,paddingHorizontal:12,flexDirection:"row",alignItems:"center",gap:7,borderWidth:1,borderColor:"#E8C9D2",borderRadius:12,backgroundColor:"#FCF4F6"},priceNeededText:{color:C.ruby,fontSize:14,fontWeight:"700"},facts:{flexWrap:"wrap",gap:14,marginTop:10,paddingTop:10,flexDirection:"row",justifyContent:"space-between",borderTopWidth:1,borderTopColor:C.border},factLabel:{color:C.muted,fontSize:12,fontWeight:"700",letterSpacing:.5},factValue:{marginTop:3,color:C.ink,fontSize:14,fontWeight:"700"},depositLine:{marginTop:10,fontSize:14,fontWeight:"700"},paymentDetail:{marginTop:5,color:C.muted,fontSize:13,fontWeight:"600"},remarks:{marginTop:10,padding:10,borderRadius:9,backgroundColor:C.pale},remarksText:{marginTop:4,color:C.ink,fontSize:14,lineHeight:20},next:{alignSelf:"flex-start",maxWidth:"100%",minHeight:44,marginTop:11,paddingHorizontal:14,flexDirection:"row",alignItems:"center",justifyContent:"center",gap:10,borderRadius:11,backgroundColor:C.navy},nextDisabled:{backgroundColor:C.ruby},nextText:{flexShrink:1,color:C.white,fontSize:14,lineHeight:19,fontWeight:"700"},cardActions:{minHeight:42,flexDirection:"row",flexWrap:"wrap",alignItems:"center",gap:16},link:{minHeight:38,flexDirection:"row",alignItems:"center",gap:5},linkText:{color:C.navy,fontSize:13,fontWeight:"700"},cancelText:{color:C.ruby,fontSize:13,fontWeight:"700"},
