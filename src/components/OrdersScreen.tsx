...
export function OrdersScreen({ businessId, locationId }: { businessId: string; locationId: string }) {
  ...
  const save = async () => {
    ...
    const payload={business_id:businessId,location_id:locationId,title:form.title.trim(),customer_name:form.customer_name.trim()||null,customer_contact:form.customer_contact.trim()||null,source,quantity,order_date:form.order_date,target_date:form.target_date||null,total_price:total,amount_paid:paid,payment_status,payment_channel:paid>0?form.payment_channel.trim()||null:null,payment_reference:paid>0?form.payment_reference.trim()||null:null,notes:form.notes.trim()||null};
...
function OrderCard({order,onEdit,onDuplicate,onStatus}:{order:Order;onEdit:()=>void;onDuplicate:()=>void;onStatus:(s:OrderStatus)=>void}) {
  ...
  return <View style={s.card}>
    ...
    <View style={[s.paymentTag,{backgroundColor:stageSoft}]}><Ionicons name={stage==="full"?"checkmark-circle":"card-outline"} size={16} color={stageColor}/><Text style={[s.paymentTagText,{color:stageColor}]}>{paymentStageLabel[stage]}</Text></View>
