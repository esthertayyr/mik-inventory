...
const C = { ink: "#101318", muted: "#626A73", navy: "#142C47", green: "#264A3B", ruby: "#65243A", amber: "#51456F", border: "#E0E3E7", pale: "#F6F7F8", white: "#FFF" };
...
function OrderCard({order,onEdit,onDuplicate,onStatus}:{order:Order;onEdit:()=>void;onDuplicate:()=>void;onStatus:(s:OrderStatus)=>void}) {
  ...
  return <View style={[s.card,compact&&s.cardMobile]}>
    ...
    <View style={s.glanceRow}>
      <View style={s.glanceItem}><Text style={s.factLabel}>DUE</Text><Text style={[s.factValue,due&&{color:due.color}]}>{displayDate(order.target_date)||"Not set"}</Text>{due&&due.color!==C.green?<Text numberOfLines={1} style={[s.glanceNote,{color:due.color}]}>{due.text}</Text>:null}</View>
      <View style={s.glanceItem}><Text style={s.factLabel}>PAYMENT</Text><Text style={[s.factValue,{color:stageColor}]}>{paymentStageLabel[stage]}</Text><Text numberOfLines={1} style={s.glanceNote}>{priceMissing?"Add full price":balance>0?`${peso(Math.max(0,balance))} left`:"Nothing left to pay"}</Text></View>
    </View>
    ...
    <View style={s.cardActions}><Pressable accessibilityRole="button" accessibilityState={{expanded:showDetails}} style={s.link} onPress={()=>setShowDetails(!showDetails)}><Text numberOfLines={1} style={s.linkText}>{showDetails?"Hide details":"More details"}</Text></Pressable><View style={s.actionDivider}/><Pressable style={s.link} onPress={onEdit}><Text numberOfLines={1} style={s.linkText}>View / edit</Text></Pressable><View style={s.actionDivider}/><Pressable style={s.link} onPress={onDuplicate}><Text numberOfLines={1} style={s.linkText}>Duplicate</Text></Pressable><View style={s.actionDivider}/>{!["completed","cancelled"].includes(order.status)?<Pressable style={s.link} onPress={()=>onStatus("cancelled")}><Text numberOfLines={1} style={s.cancelText}>Stop</Text></Pressable>:order.status==="cancelled"?<Pressable style={s.link} onPress={()=>onStatus("new")}><Text numberOfLines={1} style={s.linkText}>Resume</Text></Pressable>:null}</View>
  </View>;
}
