...
const emptyForm = (): Form => ({ title: "", customer_name: "", customer_contact: "", source: "Social media", social_platform: "Facebook", custom_source: "", quantity: "1", order_date: displayDate(isoDate()), target_date: "", total_price: "", amount_paid: "", payment_channel: "", payment_reference: "", notes: "", image_uri: "", fulfilment_method:"collection", is_past_order:false });
...
function dueInfo(order: Order) {
  if (order.is_past_order || !order.target_date || order.status === "completed" || order.status === "cancelled") return null;
...
export function OrdersScreen({ businessId, locationId }: { businessId: string; locationId: string }) {
  ...
  const save = async () => {
    ...
    if(form.is_past_order&&paid<total) return Alert.alert("Full payment needed","A past order is saved as completed. Enter the full amount received. If payment or work is still pending, choose New or current order.");
    ...
    const payload={business_id:businessId,location_id:locationId,title:form.title.trim(),customer_name:form.customer_name.trim()||null,customer_contact:form.customer_contact.trim()||null,source,quantity,order_date:orderDate,target_date:form.is_past_order?null:targetDate,total_price:total,amount_paid:paid,payment_status,payment_channel:paid>0?form.payment_channel.trim()||null:null,payment_reference:paid>0?form.payment_reference.trim()||null:null,notes:form.notes.trim()||null,fulfilment_method:form.fulfilment_method,is_past_order:form.is_past_order,...(form.is_past_order?{status:"completed" as OrderStatus,fulfilled_at:`${orderDate}T12:00:00+08:00`}:{})};
    ...
    if(id && !form.is_past_order && paid>=total*.5) {
    ...
    Alert.alert("Order saved",form.is_past_order?"The past order and full payment are recorded in Completed orders.":paid>=total?"Full payment is recorded.":paid>=total*.5?"The downpayment is recorded. The order is pending for printing.":`Waiting for downpayment. Printing cannot start until at least ${peso(total*.5)} is paid.`);
  };
