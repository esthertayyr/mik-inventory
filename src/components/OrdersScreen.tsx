...
export function OrdersScreen({ businessId, locationId }: { businessId: string; locationId: string }) {
  ...
  const save = async () => {
    ...
    Alert.alert(
      "Order saved",
      paid >= total * .5
        ? "The 50% downpayment is recorded. Printing can begin."
        : `Printing is on hold until at least ${peso(total * .5)} is paid.`,
    );
  };
...
function OrderCard({order,onEdit,onDuplicate,onStatus}:{order:Order;onEdit:()=>void;onDuplicate:()=>void;onStatus:(s:OrderStatus)=>void}) {
  ...
  const displayStatus=order.status==="cancelled"?"Stopped":order.status==="completed"?"Completed":order.status==="ready"?"Ready":depositPaid?"Active":"Pending payment";
  ...
  return <View style={s.card}>
    ...
    <Text style={[s.depositLine,{color:depositPaid?C.green:C.ruby}]}>{depositPaid?"50% downpayment received":`${peso(Math.max(0,depositNeeded-Number(order.amount_paid)))} more downpayment needed`}</Text>
    ...
    {next?<Pressable style={[s.next,!depositPaid&&next==="making"&&s.nextDisabled]} onPress={()=>onStatus(next)}><Text style={s.nextText}>{next==="making"?(depositPaid?"Start printing":"Record 50% downpayment first"):next==="ready"?"Mark as ready":"Mark completed"}</Text><Ionicons name={depositPaid||next!=="making"?"arrow-forward":"lock-closed"} size={20} color={C.white}/></Pressable>:null}
...
function OrderForm({form,setForm,saving,editing,onBack,onSave,onPhoto}:{form:Form;setForm:Dispatch<SetStateAction<Form>>;saving:boolean;editing:boolean;onBack:()=>void;onSave:()=>void;onPhoto:()=>void}) {
  ...
  return <ScrollView contentContainerStyle={s.page}>
    ...
    <View style={s.paymentBox}><View style={s.paymentHead}><Ionicons name="card-outline" size={25} color={C.navy}/><View><Text style={s.paymentTitle}>50% downpayment</Text><Text style={s.photoHelp}>No downpayment means no printing.</Text></View></View><Field label="Amount paid" value={form.amount_paid} onChangeText={v=>update("amount_paid",v)} keyboardType="decimal-pad" placeholder="₱0"/><View style={s.paymentNumbers}><Text style={s.balance}>Needed: {peso(total*.5)}</Text><Pressable style={s.halfPayment} onPress={()=>update("amount_paid",String(total*.5))}><Text style={s.halfPaymentText}>Use 50%</Text></Pressable></View><Text style={s.balanceSmall}>Full balance: {peso(Math.max(0,total-paid))}</Text><Text style={s.label}>Payment method</Text><View style={s.wrap}>{["Cash","GCash","Bank transfer","Facebook / online","Other"].map(x=><Pressable key={x} style={[s.choice,form.payment_channel===x&&s.choiceOn]} onPress={()=>update("payment_channel",x)}><Text style={[s.choiceText,form.payment_channel===x&&s.choiceTextOn]}>{x}</Text></Pressable>)}</View><Field label="Payment reference" value={form.payment_reference} onChangeText={v=>update("payment_reference",v)} placeholder="Optional receipt or reference"/></View>
    <Field label="Remarks" value={form.notes} onChangeText={v=>update("notes",v)} placeholder="Colour, size, design, delivery or anything important" multiline/>
...
const s=StyleSheet.create({
  ...
  card:{marginTop:12,padding:16,borderWidth:1,borderColor:C.border,borderRadius:16,backgroundColor:C.white},cardTop:{flexDirection:"row",gap:12},photo:{width:78,height:78,borderRadius:12,resizeMode:"contain",backgroundColor:C.pale},noPhoto:{width:78,height:78,alignItems:"center",justifyContent:"center",borderRadius:12,backgroundColor:C.pale},cardMain:{flex:1},numberRow:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:6},number:{color:C.muted,fontSize:11,fontWeight:"700",letterSpacing:.7},status:{paddingHorizontal:9,paddingVertical:6,flexDirection:"row",alignItems:"center",gap:4,borderRadius:12,backgroundColor:C.pale},statusHold:{backgroundColor:"#FBEFF1"},statusText:{color:C.navy,fontSize:11,fontWeight:"700"},cardTitle:{marginTop:7,color:C.ink,fontSize:19,lineHeight:24,fontWeight:"700"},meta:{marginTop:5,color:C.muted,fontSize:13,fontWeight:"600"},facts:{marginTop:14,paddingTop:12,flexDirection:"row",justifyContent:"space-between",borderTopWidth:1,borderTopColor:C.border},factLabel:{color:C.muted,fontSize:10,fontWeight:"700",letterSpacing:.8},factValue:{marginTop:4,color:C.ink,fontSize:13,fontWeight:"700"},depositLine:{marginTop:11,fontSize:13,fontWeight:"700"},paymentDetail:{marginTop:5,color:C.muted,fontSize:12,fontWeight:"700"},remarks:{marginTop:12,padding:11,borderRadius:9,backgroundColor:C.pale},remarksText:{marginTop:4,color:C.ink,fontSize:13,lineHeight:19},next:{minHeight:52,marginTop:13,paddingHorizontal:16,flexDirection:"row",alignItems:"center",justifyContent:"space-between",borderRadius:12,backgroundColor:C.navy},nextDisabled:{backgroundColor:C.ruby},nextText:{color:C.white,fontSize:15,fontWeight:"700"},cardActions:{minHeight:44,flexDirection:"row",alignItems:"center",gap:18},link:{flexDirection:"row",alignItems:"center",gap:5},linkText:{color:C.navy,fontSize:12,fontWeight:"700"},cancelText:{color:C.ruby,fontSize:12,fontWeight:"700"},
  ...
  back:{minHeight:48,flexDirection:"row",alignItems:"center",gap:9},backText:{color:C.ink,fontSize:17,fontWeight:"700"},formLead:{marginTop:10,color:C.ink,fontSize:28,lineHeight:34,fontWeight:"700",letterSpacing:-.7},photoPicker:{minHeight:170,marginTop:18,alignItems:"center",justifyContent:"center",overflow:"hidden",borderWidth:1,borderStyle:"dashed",borderColor:C.border,borderRadius:12,backgroundColor:C.pale},formPhoto:{width:"100%",height:230,resizeMode:"contain"},photoCircle:{width:52,height:52,alignItems:"center",justifyContent:"center",borderRadius:26,backgroundColor:C.navy},photoTitle:{marginTop:10,color:C.ink,fontSize:16,fontWeight:"700"},photoHelp:{marginTop:3,color:C.muted,fontSize:12,lineHeight:17},label:{marginTop:15,marginBottom:6,color:C.ink,fontSize:13,fontWeight:"700"},input:{minHeight:54,paddingHorizontal:14,borderWidth:1,borderColor:C.border,borderRadius:10,backgroundColor:C.white,color:C.ink,fontSize:16},notes:{minHeight:110,paddingTop:14,textAlignVertical:"top"},two:{flexDirection:"row",gap:10},half:{flex:1},wrap:{flexDirection:"row",flexWrap:"wrap",gap:7},choice:{minHeight:42,paddingHorizontal:13,alignItems:"center",justifyContent:"center",borderWidth:1,borderColor:C.border,borderRadius:9,backgroundColor:C.white},choiceOn:{borderColor:C.navy,backgroundColor:C.navy},choiceText:{color:C.ink,fontSize:12,fontWeight:"700"},choiceTextOn:{color:C.white},paymentBox:{marginTop:18,padding:15,borderWidth:1,borderColor:C.border,borderRadius:12,backgroundColor:C.pale},paymentHead:{flexDirection:"row",alignItems:"center",gap:10},paymentTitle:{color:C.ink,fontSize:18,fontWeight:"700"},paymentNumbers:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:10},balance:{marginTop:8,color:C.green,fontSize:19,fontWeight:"700"},balanceSmall:{marginTop:5,color:C.muted,fontSize:13,fontWeight:"700"},halfPayment:{marginTop:8,minHeight:38,paddingHorizontal:13,alignItems:"center",justifyContent:"center",borderRadius:8,backgroundColor:C.navy},halfPaymentText:{color:C.white,fontSize:12,fontWeight:"700"},save:{minHeight:58,marginTop:22,flexDirection:"row",alignItems:"center",justifyContent:"center",gap:8,borderRadius:10,backgroundColor:C.navy},saveText:{color:C.white,fontSize:16,fontWeight:"700"},
});
