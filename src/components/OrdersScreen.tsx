...
export function OrdersScreen({ businessId, locationId, initialOrderId, onOrderOpened }: { businessId: string; locationId: string; initialOrderId?:string|null; onOrderOpened?:()=>void }) {
  ...
  if(section==="enquiries") return <ScrollView contentContainerStyle={s.page}><View style={s.headingRow}><View style={s.headingCopy}><Text style={s.title}>Potential orders</Text><Text style={s.subtitle}>Save interested customers before they confirm</Text></View></View><View style={s.sectionSwitch}><Pressable style={s.sectionSwitchOff} onPress={()=>setSection("orders")}><Text style={s.sectionSwitchOffText}>Confirmed orders</Text></Pressable><View style={s.sectionSwitchOn}><Text style={s.sectionSwitchOnText}>Potential orders</Text></View></View><EnquiriesPanel businessId={businessId} locationId={locationId} onConvert={convertEnquiry}/></ScrollView>;
