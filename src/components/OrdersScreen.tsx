...
const C = { ink: "#101318", muted: "#626A73", navy: "#142C47", blue: "#315C88", green: "#264A3B", ruby: "#65243A", amber: "#9A6437", border: "#E0E3E7", pale: "#F6F7F8", white: "#FFF" };
...
export function OrdersScreen({ businessId, locationId }: { businessId: string; locationId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const [{data:o,error},{data:s}] = await Promise.all([
      supabase.from("external_orders").select("*").eq("location_id",locationId).order("created_at",{ascending:false}),
...
function OrderCard({order,onEdit,onDuplicate,onStatus}:{order:Order;onEdit:()=>void;onDuplicate:()=>void;onStatus:(s:OrderStatus)=>void}) {
  ...
  return <View style={[s.card,compact&&s.cardMobile]}>
    ...
    <View style={s.glanceRow}>
      <View style={[s.infoPill,due&&due.color!==C.green&&s.infoPillAlert]}><Ionicons name="calendar-outline" size={16} color={due?.color??C.muted}/><Text numberOfLines={1} style={[s.infoPillText,due&&{color:due.color}]}>{displayDate(order.target_date)||"No due date"}</Text></View>
    ...
    {showDetails?<View style={s.detailsPanel}><View style={s.facts}><View><Text style={s.factLabel}>FULL PRICE</Text><Text style={s.factValue}>{peso(Number(order.total_price))}</Text></View><View><Text style={s.factLabel}>PAID</Text><Text style={s.factValue}>{peso(Number(order.amount_paid))}</Text></View><View><Text style={s.factLabel}>BALANCE</Text><Text style={s.factValue}>{peso(Math.max(0,balance))}</Text></View></View>{stage!=="full"?<Text style={[s.depositLine,{color:stageColor}]}>{paymentMessage}</Text>:null}{order.amount_paid>0?<Text style={s.paymentDetail}>{peso(Number(order.amount_paid))} paid via {order.payment_channel||"payment method not set"}</Text>:null}</View>:null}
