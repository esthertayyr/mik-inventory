...
function dueInfo(order: Order) {
  ...
  return { text: "Upcoming", color: C.green };
}
...
export function OrdersScreen({ businessId, locationId }: { businessId: string; locationId: string }) {
  ...
  const save = async () => {
    ...
    if(form.target_date.trim() && (!targetDate || targetDate<orderDate)) return Alert.alert("Check due date","Use DD-MM-YYYY and choose a date on or after the order date, or leave it empty.");
  ...
  return <ScrollView contentContainerStyle={s.page}>
    ...
    <View style={s.followUpGrid}><View style={s.followUpCard}><Text style={s.followUpValue}>{followUp.collect}</Text><Text style={s.followUpLabel}>Waiting for collection</Text></View><View style={s.followUpCard}><Text style={s.followUpValue}>{followUp.payment}</Text><Text style={s.followUpLabel}>Waiting for downpayment</Text></View><View style={s.followUpCard}><Text style={s.followUpValue}>{followUp.print}</Text><Text style={s.followUpLabel}>Ready to print</Text></View></View>
...
function OrderCard({order,onEdit,onDuplicate,onStatus}:{order:Order;onEdit:()=>void;onDuplicate:()=>void;onStatus:(s:OrderStatus)=>void}) {
  ...
  const statusColor=order.status==="cancelled"?C.ruby:order.status==="ready"||order.status==="completed"?C.green:order.status==="making"?C.blue:depositPaid?C.navy:C.amber;
  const statusSoft=order.status==="cancelled"?"#F8F0F2":order.status==="ready"||order.status==="completed"?"#ECF5F0":order.status==="making"?"#EEF3F8":depositPaid?C.pale:"#FAF4EA";
  ...
  return <View style={[s.card,compact&&s.cardMobile]}>
    ...
    <View style={s.glanceRow}>
      <View style={[s.infoPill,due&&due.color!==C.green&&s.infoPillAlert]}><Ionicons name="calendar-outline" size={16} color={due?.color??C.muted}/><Text numberOfLines={1} style={[s.infoPillText,due&&{color:due.color}]}>{order.target_date?`Due date: ${displayDate(order.target_date)}`:`Order date: ${displayDate(order.order_date)}`}</Text></View>
    ...
    {showDetails?<View style={s.detailsPanel}><View style={s.paymentSummaryTop}><View style={[s.paymentSummaryIcon,{backgroundColor:stage==="full"?"#EAF4EF":"#FAF0F2"}]}><Ionicons name={stage==="full"?"checkmark":"wallet-outline"} size={20} color={stageColor}/></View><View style={s.flex}><Text style={s.paymentSummaryTitle}>{stage==="full"?"Paid in full":paymentStageLabel[stage]}</Text><Text style={s.paymentSummaryAmount}>{peso(Number(order.amount_paid))} of {peso(Number(order.total_price))}</Text></View>{balance>0?<View style={s.balanceBadge}><Text style={s.balanceBadgeLabel}>LEFT</Text><Text style={s.balanceBadgeValue}>{peso(balance)}</Text></View>:null}</View>{stage!=="full"?<View style={s.paymentProgress}><View style={[s.paymentProgressFill,{width:`${Math.min(100,Number(order.amount_paid)/Math.max(1,Number(order.total_price))*100)}%`,backgroundColor:stageColor}]}/></View>:null}<Text style={s.paymentDetail}>{order.amount_paid>0?`${order.payment_channel||"Payment method not set"}${latestPayment?` · Received ${displayDate(latestPayment.payment_date)}`:""}`:"No customer payment recorded"}</Text><Text style={s.orderDateDetail}>Order date: {displayDate(order.order_date)}{order.target_date?` · Due date: ${displayDate(order.target_date)}`:""}</Text></View>:null}
    {showDetails && order.notes?<View style={s.remarks}><Text style={s.factLabel}>REMARKS</Text><Text style={s.remarksText}>{order.notes}</Text></View>:null}
