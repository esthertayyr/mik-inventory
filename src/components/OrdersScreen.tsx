...
function OrderCard({order,onEdit,onDuplicate,onStatus}:{order:Order;onEdit:()=>void;onDuplicate:()=>void;onStatus:(s:OrderStatus)=>void}) {
  ...
  return <View style={s.card}>
    ...
    <View style={s.cardActions}><Pressable accessibilityRole="button" accessibilityState={{expanded:showDetails}} style={s.link} onPress={()=>setShowDetails(!showDetails)}><Text style={s.linkText}>{showDetails?"Hide details":"More details"}</Text></Pressable><View style={s.actionDivider}/><Pressable style={s.link} onPress={onEdit}><Text style={s.linkText}>View / edit</Text></Pressable><View style={s.actionDivider}/><Pressable style={s.link} onPress={onDuplicate}><Text style={s.linkText}>Duplicate</Text></Pressable><View style={s.actionDivider}/>{!["completed","cancelled"].includes(order.status)?<Pressable style={s.link} onPress={()=>onStatus("cancelled")}><Text style={s.cancelText}>Stop</Text></Pressable>:order.status==="cancelled"?<Pressable style={s.link} onPress={()=>onStatus("new")}><Text style={s.linkText}>Resume</Text></Pressable>:null}</View>
  </View>;
}
