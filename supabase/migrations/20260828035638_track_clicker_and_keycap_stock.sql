-- Keyboard Clickers use two inventories at once: one empty base from the exact
-- slot-size product, plus the selected letters from the shared alphabet pool.
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    if v_has_choices then
      ...
      if v_product.letters_required<=0 or v_product.name like 'Keyboard Clicker%' then
    ...
    if (v_product.letters_required<=0 or v_product.name like 'Keyboard Clicker%') and coalesce(v_stock,0)<v_quantity
      then raise exception 'Only % unit(s) available for %',coalesce(v_stock,0),v_product.name; end if;
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    elsif v_product.letters_required<=0 or v_product.name like 'Keyboard Clicker%' then
...
-- Remove the old non-blocking placeholder of 1000 bases. Preserve any real
-- count below that value and flag legacy rows for the owner to count once.
...
where p.id = il.product_id
  and p.name like 'Keyboard Clicker%'
  and p.letters_required > 0
  and il.quantity_on_hand >= 1000;
